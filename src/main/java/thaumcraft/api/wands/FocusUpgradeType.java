...
public class FocusUpgradeType {
    ...
    /**
     * What aspects are used to calculate the cost of this upgrade. The amounts given is ignored, just the type is used
     * for the calculation.
     */
    ...
    public FocusUpgradeType(int id, ResourceLocation icon, String name, String text, AspectList aspects) {
        ...
        if (id < types.length && types[id] != null) {
            LogManager.getLogger("THAUMCRAFT")
                .fatal("Focus Upgrade id " + id + " already occupied. Ignoring.");
            return;
        }
