...
public final class AspectUtils {
     ...
     */
    static final Map<String, AspectList> CACHE = new HashMap<>();
     ...
     */
    static final Set<String> FAILED_IDS = new TreeSet<>();
     ...
     */
    static {
        putAspect("aer", Aspect.AIR);
        putAspect("ignis", Aspect.FIRE);
        putAspect("aqua", Aspect.WATER);
        putAspect("terra", Aspect.EARTH);
        putAspect("ordo", Aspect.ORDER);
        putAspect("perditio", Aspect.ENTROPY);

        String[] tags = { "vacuos", "lux", "tempestas", "motus", "gelum", "vitreus", "victus", "venenum", "potentia",
            "permutatio", "metallum", "mortuus", "volatus", "tenebrae", "spiritus", "sano", "iter", "alienis",
            "praecantatio", "auram", "vitium", "limus", "herba", "arbor", "bestia", "corpus", "exanimis", "cognitio",
            "sensus", "humanus", "messis", "perfodio", "instrumentum", "meto", "telum", "tutamen", "fames", "lucrum",
            "fabrico", "pannus", "machina", "vinculum", "strontio", "nebrisum", "electrum", "magneto", "radio",
            "alkimia", "luxuria", "infernus", "superbia", "gula", "invidia", "desidia", "ira", "terminus", "tempus",
            "humilitas", "dimensio", "sanguino", "gravitas", "energy", "day", "night", "mattery", "alfirin", "abonnen",
            "edhel", "nauglin", "orchoth", "perian", "mornogol", "torog", "onodrim", "uhorm", "draugol", "valaraukar",
            "ithryn", "shire", "dunedain", "eredluin", "lindon", "gundabad", "angmar", "druardh", "dolguldur", "dale",
            "angdol", "lothlorien", "dunland", "isengard", "fangorn", "rohan", "gondor", "mordor", "dorwinion",
            "rhudel", "harad", "morwaith", "taurethrim", "pertorog", "utumno", "nazgul", "bree", "caelum", "tabernus",
            "contritio", "tyranny", "rubus", "excubitor", "coralos", "dreadia", "perplexus", "signum", "principia",
            "mru", "radiation", "matrix", "odachi", "proud", "dragon", "substance", "destroy", "universe", "space",
            "mana", "relic", "treasure", "evil", "dackmagic", "dream", "time", "rock", "paper", "lava", "magnetic",
            "electricity", "enchant", "alloy", "gravity", "vegetation", "cave", "darkenergy", "antimatter",
        ...
        for (String tag : tags) {
            putAspect(tag, Aspect.getAspect(tag));
        }
    }
     ...
     */
    private static void putAspect(String tag, Aspect aspect) {
        if (aspect != null) {
            ASPECT_MAP.put(tag, aspect);
        } else {
    ...
    public static Aspect getAspect(String tag) {
        Aspect a = ASPECT_MAP.get(tag);
        if (a == null) {
            a = Aspect.getAspect(tag);
            if (a != null) ASPECT_MAP.put(tag, a);
        }
        return a;
    }
    ...
    public static boolean hasAspect(ItemStack stack) {
        ...
        AspectList al = ThaumcraftApiHelper.getObjectAspects(stack);
     ...
     */
    public static AspectList scaleAspects(AspectList src, float factor) {
        if (src == null || src.size() == 0) return null;
        Aspect[] aspects = src.getAspects();
        if (aspects == null || aspects.length == 0) return null;
        AspectList res = new AspectList();
        for (Aspect a : aspects) {
            if (a == null) continue;
            int amt = src.getAmount(a);
            if (amt <= 0) continue;
            int scaled = Math.max(1, Math.round(amt * factor));
            res.add(a, scaled);
        }
     ...
     */
    public static int getAspectTotal(AspectList al) {
        if (al == null) return 0;
        Aspect[] aspects = al.getAspects();
        if (aspects == null || aspects.length == 0) return 0;
        int total = 0;
        for (Aspect a : aspects) {
            if (a == null) continue;
     ...
     */
    public static AspectList mergeAspectsMax(AspectList a, AspectList b) {
        if (a == null && b == null) return null;
        if (a == null) return b.copy();
        if (b == null) return a.copy();
        AspectList out = new AspectList();
        java.util.Set<Aspect> seen = new java.util.HashSet<>();
        Aspect[] aa = a.getAspects();
        if (aa != null) {
            for (Aspect asp : aa) {
                if (asp == null) continue;
                seen.add(asp);
                int amtA = a.getAmount(asp);
                int amtB = b.getAmount(asp);
                out.add(asp, Math.max(amtA, amtB));
            }
        }
        Aspect[] ab = b.getAspects();
        if (ab != null) {
            for (Aspect asp : ab) {
                if (asp == null || seen.contains(asp)) continue;
                int amtA = a.getAmount(asp);
                int amtB = b.getAmount(asp);
                out.add(asp, Math.max(amtA, amtB));
            }
     ...
     */
    public static AspectList getBestFromSlot(List<ItemStack> options, int depth, Set<String> visiting) {
        AspectList best = null;
        int bestScore = -1;

        for (ItemStack opt : options) {
            if (opt == null) continue;
            AspectList cur = AspectDeriver.getOrGenerateAspectsFor(opt, depth, visiting);
            if (cur == null || cur.size() == 0) continue;

            int score = getAspectTotal(cur);
            if (score > bestScore) {
                bestScore = score;
                best = cur;
            }
     ...
     */
    private static void invokeGetSubItems(Item item, List<ItemStack> outList) {
        Method m = findGetSubItemsMethod(item.getClass());
        if (m == null) return;
        try {
    ...
    private static Method findGetSubItemsMethod(Class<?> start) {
        try {
            for (String name : new String[] { "getSubItems", "func_150895_a" }) {
                for (Class<?> c = start; c != null && Item.class.isAssignableFrom(c); c = c.getSuperclass()) {
                    try {
                        Method m = c.getDeclaredMethod(name, GET_SUB_ITEMS_PARAMS);
    ...
    public static Set<Integer> getMetasToScan(Item item) {
        ...
        if (isClientSide()) {
            try {
                ...
                invokeGetSubItems(item, subItems);
                for (ItemStack sub : subItems) {
                    if (sub != null && sub.getItem() == item) {
                        int m = sub.getItemDamage();
        ...
        if (ORE_DICT_METAS != null) {
            Set<Integer> oreMetas = ORE_DICT_METAS.get(item);
            if (oreMetas != null) metas.addAll(oreMetas);
        }
        ...
        if (RECIPE_OUTPUT_METAS != null) {
            Set<Integer> recipeMetas = RECIPE_OUTPUT_METAS.get(item);
            if (recipeMetas != null) metas.addAll(recipeMetas);
        }

        // Source 4: Fallback 0-15 — for items that declare hasSubtypes but none of the
        // above sources found any metas beyond 0. Scans 0-15 as a safety net (covers
        // vanilla-style items like dyes, wool colors, wood types, etc.).
        // 来源 4：兜底 0-15 —— 对于声明 hasSubtypes 但以上来源均未找到 0 以外 meta 的物品，
        // 扫描 0-15 作为安全网（覆盖染料、羊毛颜色、木材类型等原版风格物品）。
        if (item.getHasSubtypes() && metas.size() <= 1) {
            for (int i = 0; i < 16; i++) {
                metas.add(i);
            }
        }
     ...
     */
    public static ItemStack getFluidRepresentative(Fluid fluid) {
        ...
        for (FluidContainerRegistry.FluidContainerData d : FluidContainerRegistry.getRegisteredFluidContainerData()) {
            if (d != null && d.fluid != null && d.fluid.getFluid() == fluid && d.filledContainer != null) {
     ...
     */
    public static boolean isFoodLike(Item item, ItemStack stack) {
     ...
     */
    public static AspectList generateWithThaumcraft(ItemStack stack) {
        try {
            AspectList al = ThaumcraftApiHelper.generateTags(stack.getItem(), stack.getItemDamage());
     ...
     */
    public static List<List<ItemStack>> getRecipeInputs(IRecipe recipe) {
        List<List<ItemStack>> inputs = new ArrayList<>();

        // 1. Standard recipe types (known class structure, no reflection needed)
        // 标准配方类型（已知类结构，不需要反射）
        if (recipe instanceof ShapedRecipes sr) {
            for (ItemStack s : sr.recipeItems) {
                if (s != null) inputs.add(Collections.singletonList(s.copy()));
            }
        } else if (recipe instanceof ShapedOreRecipe sor) {
            for (Object o : sor.getInput()) {
                inputs.add(resolveOreInput(o));
            }
        } else if (recipe instanceof ShapelessRecipes slr) {
            @SuppressWarnings("unchecked")
            List<ItemStack> items = slr.recipeItems;
        ...
        if (inputs.isEmpty()) {
            inputs = extractInputsViaReflection(recipe);
        }
     ...
     * 跳过类似输出的字段以避免误报。
     */
    private static List<List<ItemStack>> extractInputsViaReflection(IRecipe recipe) {
        List<List<ItemStack>> inputs = new ArrayList<>();

        // Strategy 1: Try common getter method names that mod recipe classes often expose.
        // 策略1：尝试模组配方类常暴露的 getter 方法名。
        String[] methodNames = { "getInput", "getIngredients", "getInputs", "getCraftingInput", "getComponents" };
        for (String name : methodNames) {
            try {
                Method m = recipe.getClass()
                    .getMethod(name);
                Object result = m.invoke(recipe);
                collectInputsFromObject(result, inputs);
        ...
        while (clazz != null && clazz != Object.class) {
            for (Field f : clazz.getDeclaredFields()) {
                try {
                    ...
                    collectInputsFromObject(val, inputs);
                    if (!inputs.isEmpty()) return inputs;
                } catch (Exception ignored) {}
            }
     ...
     */
    private static void collectInputsFromObject(Object obj, List<List<ItemStack>> inputs) {
        if (obj instanceof ItemStack[]arr) {
            ...
            for (Object o : arr) {
                List<ItemStack> resolved = resolveOreInput(o);
     ...
     */
    public static List<ItemStack> resolveOreInput(Object input) {
        ...
        if (input instanceof ItemStack) list.add(((ItemStack) input).copy());
            ...
            Item i = Item.getItemFromBlock((Block) input);
            if (i != null) list.add(new ItemStack(i, 1, OreDictionary.WILDCARD_VALUE));
        } else if (input instanceof String) list.addAll(OreDictionary.getOres((String) input));
        else if (input instanceof List) {
            for (Object sub : (List<?>) input) list.addAll(resolveOreInput(sub));
        }
        return list;
    }
     ...
     */
    public static String aspectListToString(AspectList al) {
        if (al == null) return "[null]";
        try {
            Aspect[] aspects = al.getAspects();
            if (aspects == null || aspects.length == 0) return "[empty, size=" + al.size() + "]";
     ...
     */
    public static int loadAspectCacheFromFile(File file) {
        ...
        try (BufferedReader r = new BufferedReader(
            ...
            while ((line = r.readLine()) != null) {
                ...
                String key = line.substring(0, eq)
                    .trim();
                String val = line.substring(eq + 3)
                    .trim();
                int at = key.indexOf('@');
                if (at <= 0) continue;
    ...
    private static AspectList parseAspectList(String s) {
        ...
        for (String part : s.split(",")) {
            ...
            Aspect a = getAspect(tag);
            if (a != null && amt > 0) al.add(a, amt);
        }
